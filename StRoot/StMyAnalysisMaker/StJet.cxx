 ...
 */
StJet::StJet(Double_t px, Double_t py, Double_t pz) : StVParticle(), // FIXME
 ...
 */
StJet::StJet(Double_t pt, Double_t eta, Double_t phi, Double_t m) : StVParticle(), // FIXME
                                                                    fPt(pt),
                                                                    fEta(eta),
                                                                    fPhi(phi),
                                                                    fM(m),
 ...
 */
StJet::StJet(const StJet &jet) : StVParticle(jet), // FIXME
                                 fPt(jet.fPt),
                                 fEta(jet.fEta),
                                 fPhi(jet.fPhi),
                                 fM(jet.fM),
...
StJet::~StJet()
{
  if (fJetShapeProperties)
    delete fJetShapeProperties;
}
...
Double_t StJet::GetZD0(vector<TLorentzVector>& tracks) const
{
  Double_t pJetSq = P();
  pJetSq *= pJetSq;
  Double_t trkPx=0., trkPy=0., trkPz=0.;

   for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
    Int_t uid = particle->user_index();

    // CHARGED COMPONENT (tracks)
      if (int(uid) >= 10000 && int(uid) < 20000) // this is a charged track id range
      {
        TLorentzVector v = tracks[uid - 10000];
        if ((int)v.M() == 1)
        {
        trkPx = v.Px();
        trkPy = v.Py();
...
Double_t StJet::GetAngularityLambda(const Double_t &kappa, const Double_t &alpha) const
{
  Double_t pJet = Pt();
  Double_t lambda = 0.;
 // Double_t SumPt = 0.;
 // const vector<fastjet::PseudoJet> &constituents = fJetConstit;
  ...
  for (vector<fastjet::PseudoJet>::const_iterator particle = fJetConstit.begin(); particle != fJetConstit.end(); ++particle)
  {
    ...
    if (uid >= 0 && uid < 20000)
    {
      ...
      Double_t dEta = Eta() - particle->eta();
      dPhi = TVector2::Phi_mpi_pi(dPhi);
      Double_t delta_R = TMath::Sqrt(dPhi * dPhi + dEta * dEta);
      lambda += pow(particle->pt() / pJet, kappa) * pow(delta_R, alpha);
    }
  }
  return lambda;
}
