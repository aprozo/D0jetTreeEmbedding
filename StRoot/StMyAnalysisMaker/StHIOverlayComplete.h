#ifndef StHIOverlayComplete_h
#define StHIOverlayComplete_h
...
class StHIOverlayComplete : public StJetFrameworkPicoBase
{
public:
  StHIOverlayComplete(const char *name, StPicoDstMaker *picoMaker, const char *outName, const char *filename);
  virtual ~StHIOverlayComplete();
  ...
  virtual void SetACentralityBinForTest(int i)
  {
    if (i >= 0 && i <= 2)
      fCentBin = i;
  }
  ...
  void DoTrackingEfficiency(double percentage)
  {
...
private:
  ...
  double fTrackingEfficiencyPercentage; // percentage of tracks to keep

  ClassDef(StHIOverlayComplete, 1)
};
#endif
